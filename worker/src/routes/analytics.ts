...
router.get('/dashboard', async (c) => {
  ...
  const thisMonthRow = await c.env.DB.prepare(
    `SELECT SUM(gross_amount) as revenue, COUNT(*) as count FROM transactions WHERE strftime('%Y-%m', payment_date) = ?`
  ).bind(currentMonth).first<{ revenue: number | null; count: number }>();

  const lastMonthRow = await c.env.DB.prepare(
    `SELECT SUM(gross_amount) as revenue FROM transactions WHERE strftime('%Y-%m', payment_date) = ?`
  ).bind(lastMonth).first<{ revenue: number | null }>();
...
router.get('/monthly-revenue', async (c) => {
...
router.get('/category-breakdown', async (c) => {
  const row = await c.env.DB.prepare(`
    SELECT
...
router.get('/cumulative-earnings', async (c) => {
  const { results } = await c.env.DB.prepare(`
    SELECT
      strftime('%Y-%m', payment_date) as month,
    ...
    GROUP BY strftime('%Y-%m', payment_date)
    ORDER BY month ASC
  `).all<{ month: string; malachi_month: number; daniel_month: number }>();
...
router.get('/monthly-delta', async (c) => {
  const { results } = await c.env.DB.prepare(`
    SELECT
      strftime('%Y-%m', payment_date) as month,
